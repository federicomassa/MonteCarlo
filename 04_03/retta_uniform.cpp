...
void retta_uniform() {
  ...
  m_ML = sum_yx/sum_x2/camp;
  std::cout << "m_ML = " << m_ML << std::endl;

  //calcolo chi2
  for (int i = 0; i < dim; i++){
      for(int j = 0; j < camp; j++) {

	chi2 += TMath::Power((y_g[i][j]-m_ML*x[i]),2)/mean_s2;

      }
    }
	 std::cout << "Chi2/Dof: " << chi2 << "/" << (dim*camp-1) << std::endl;
	 std::cout << "Prob Chi2: " << TMath::Prob(chi2,dim*camp-1) << '\n' << std::endl;
  //GRAFICO LIKELIHOOD//
  double func_L[101];
  double func_m[101];
